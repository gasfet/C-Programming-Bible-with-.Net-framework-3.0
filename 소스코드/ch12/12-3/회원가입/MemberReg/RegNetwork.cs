...
namespace MemberReg
{
	...
	public class RegNetwork
	{
		...
		public RegNetwork(Reg_Form wnd)
		{				
			this.wnd = wnd;			
		}
		...
		public void Receive()
		{
			try
			{
					...
					switch(token[0].Trim())
					{
						case "STOC_MESSAGE_REGISTER_OK":   // 회원 가입 성공  
							System.Windows.Forms.MessageBox.Show("회원 가입 성공! \r\n 새로 로그인 해주세요!");                                                 
							this.wnd.Close();  // 회원 가입창 닫기
							break;
                      
						case "STOC_MESSAGE_REGISTER_FAIL": // 회원 가입 실패 
							System.Windows.Forms.MessageBox.Show("회원 가입 실패!");
							this.wnd.Close();  // 회원 가입창 닫기
							break;

						case "STOC_MESSAGE_REGISTER_IDYES": //아이디 중복조회 결과 사용가능
							System.Windows.Forms.MessageBox.Show("아이디 사용가능!");
							this.wnd.id_ok = true;
							break;

						case "STOC_MESSAGE_REGISTER_IDNO":  //아이디 중복조회 결과 사용불가
							System.Windows.Forms.MessageBox.Show("아이디 사용불가!");
							this.wnd.id_ok = false;
							break;

						case "STOC_MESSAGE_REGISTER_ZIPCODEDATA": // 우편번호 조회 결과
							this.wnd.zipcodewnd.ZipdataInput(token[1].Trim());
							break;

						case "STOC_MESSAGE_REGISTER_ZIPCODERR": // 우편번호 조회 에러
