...
class XmlReaderSettingsExam
{
    static void Main(string[] args)
    {
        ...
        XmlTextReader xr = new XmlTextReader(str_dtdname);
        XmlReaderSettings settings = new XmlReaderSettings();
        settings.ValidationType = ValidationType.DTD;
        settings.ProhibitDtd = false;
        settings.ValidationEventHandler += new System.Xml.Schema.ValidationEventHandler(xvr_ValidationEventHandler);
        XmlReader reader = XmlReader.Create(str_dtdname, settings);
        while (reader.Read()) ;
    }
    public static void xvr_ValidationEventHandler(object sender, ValidationEventArgs args)
    {
